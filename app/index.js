...
function listenInputMessages() {
  try {
    moduleClient.on("inputMessage", function (inputName, message) {
      filterMessage(moduleClient, inputName, message);
    });
  } catch (e) {
    console.error(e);
    setTimeout(listenInputMessages, 5000);
  }
}
...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    fullscreen: process.env.CDH_FULLSCREEN || false,
    alwaysOnTop: false,
    frame: process.env.CDH_FRAME || false,
    webPreferences: {
